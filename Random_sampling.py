...
for mass in StarClusterMasses:
    for metallicity in M_over_Hs:
        ...
        with open(f'metallicity_{metallicity}/Random_sampled_star_cluster_{metallicity}_{np.round(mass, 1)}_ini.txt', 'w') as file:
                writer = csv.writer(file, delimiter=' ')
                writer.writerow(np.array([np.round(metallicity, 3), np.round(mass, 2)]))
